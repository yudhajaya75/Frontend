        ...
        {loading ? (
          ...
          <div className="relative z-10">
            {content.slice(0, 3).map((res: any, index: number) => (
              <div
                key={index}
                className="grid grid-cols-1 content-start w-[1300px] h-auto relative"
              >
                <div className="relative left-[50px] top-[120px]">
                  <a href={`/blog/${res.slug}`}>
                    <img
                      className="h-[400px] w-[430px] aspect-auto object-cover"
                      src={res.image_articel}
                      alt=""
                    />
                  </a>
                  <div className="w-[400px] h-[400px] border-2 bg-white p-5 rounded-lg z-10 relative left-[13px] bottom-[200px]">
                    <p className="font-bold text-[25px] line-clamp-3">
                      {res.title}
                    </p>
                    <p
                      className="relative top-4 text-[25px] line-clamp-3"
                ...
                <div className="border-l-2 h-[780px] absolute left-[42%] bottom-[0px]"></div>
              </div>
            ))}
          </div>
        )}
      ...
      <div>
        {loading ? (
          ...
          <div className="-ml-3">
            {content.slice(0, 3).map((res: any, index: number) => (
              <div
                className="grid grid-cols-1 content-start h-auto"
                key={index}
              >
                <div className=" relative left-[30px] top-[50px]">
                  <a href="/beritabaru">
                    <img
                      className="h-[122px] w-[137px]"
                      src={res.image_articel}
                      alt=""
                    />
                  </a>
                  <div className="w-[130px] h-[170px] border-2 bg-white p-3 rounded-lg z-10 relative left-[3px] bottom-[50px]">
                    <p className="font-bold text-[10px] line-clamp-3">
                      {res.title}
                    </p>
                    <p
                      className="relative top-4 text-justify text-[10px] line-clamp-3"
                      dangerouslySetInnerHTML={{ __html: res.body }}
                    ></p>
