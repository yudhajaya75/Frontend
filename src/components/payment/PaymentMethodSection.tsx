...
const PaymentMethodSection: React.FC<PaymentMethodSectionProps> = ({ index, activePage, handlePrevious }) => {
    ...
    const handleNext = async () => {
        ...
        try {
            // Create a transaction using the selected product and payment method
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/transactions?populate=*&filters[users][email][$eq]=$[USEREMAIL]`,
                {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Terjadi kesalahan saat melakukan transaksi',
            });
        }
    };
    ...
    useEffect(() => {
        if (index === 0) {
            axios
                .get(`${process.env.REACT_APP_API_URL}/product-variants/${id}?populate=*`)
                .then((response) => {
            ...
            sm-440:w-[160px] outline-none no-underline rounded-md border border-[#8D8D8D]'
                    ...
                    >
                        ...
                        {contents.map((item: any) => (
                            <option key={item.id} value={item.attributes.PaymentsBank}>
                ...
                <div className='border-solid border-2 border-[#79ABFF] mt-[-400px] w-[460px] h-[354px]'>
                    <div className='mx-5 my-5'>
                        ...
                        <p className='text-[16px] text-[#002157]'>
                            SubTotal{' '}
                            <p className='float-right'>Rp.{price ? price : 'Harga tidak tersedia'}</p>
                        </p>
                        <div className='border-b-2 border-slate-950'></div>
                        <div className='my-2 text-[#002157]'>
                            <p className='text-[16px] font-bold'>
                                Total <p className='float-right'>Rp.{price ? price : 'Harga tidak tersedia'}</p>
                            </p>
                            <br />
