              ...
              <ul className="flex items-center justify-center sm-440:flex">
                {pages.map((page, index) => (
                  <li
                    ...
                    className={`ml-5 lg:text-[24px] sm-440:text-[10px] sm:text-[18px] sm-440:font-bold p-5 ${activePage === index ? 'text-[#ffffff] bg-[#002157]' : 'text-[#002157]'
                      } rounded-md cursor-pointer duration-500`}
                    onClick={() => setActivePage(index)}
                  >
                    <a id='hyperlink'
                      href={`#${page}`}>{page.charAt(0).toUpperCase() + page.slice(1)}</a>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {hyperlink.map((res, index: number) => (
              <div
                ...
                <div className='flex items-center justify-center relative ml-[300px] mt-[80px] lg:mx-[300px] sm-440:mx-20 sm-440:bottom-[50px]'>
                  <img src={`${process.env.REACT_APP_UPLOAD_URL}${res.attributes.image.data.attributes.url}`} alt="image" className="lg:w-[400px] sm-440:w-[120px] sm:w-[200px] md:w-[300px] aspect-auto object-cover" />
                  <div className='lg:ml-10 sm-440:ml-3'>
                    <h1 className='lg:text-[32px] sm-440:text-[12px] sm:text-[25px] lg:w-[500px] font-bold text-[#002157]' dangerouslySetInnerHTML={{ __html: res.attributes.title }}></h1>
                    <br />
                    <p className='lg:text-[24px] lg:w-[500px] sm-440:text-[12px] sm:text-[20px]' dangerouslySetInnerHTML={{ __html: res.attributes.body }}></p>
                    <br />
